...
const SCIENCE_TECH_EVENTS = [
  ...
  },
  {
    ...
    lastCheckedAt: "2026-08-17T17:00:00-05:00",
    tags: ["inteligencia artificial", "datos abiertos", "software libre", "explora"],
    score: 97
  },
  {
    id: "ciencia-tech-03",
    ...
    lastCheckedAt: "2026-08-17T17:00:00-05:00",
    tags: ["python", "inteligencia artificial", "ruta n", "programacion", "tech"],
    score: 95
  },
  {
    id: "ciencia-tech-04",
  ...
  },
  {
    ...
    lastCheckedAt: "2026-08-17T17:00:00-05:00",
    tags: ["creative coding", "glsl", "arte generativo", "programacion", "shaders"],
    score: 96
  },
  {
    id: "ciencia-tech-06",
    ...
    lastCheckedAt: "2026-08-17T17:00:00-05:00",
    tags: ["robotica", "drones", "makerspace", "laureles", "ingenieria"],
    score: 92
  },
  {
    id: "ciencia-tech-07",
  ...
  },
  {
    ...
    lastCheckedAt: "2026-08-17T17:00:00-05:00",
    tags: ["iot", "hardware", "esp32", "rionegro", "oriente", "agricultura tech"],
    score: 95
  }
];

async function run() {
  console.log(`Adding ${SCIENCE_TECH_EVENTS.length} science and technology events...`);
  ...
  const newIds = new Set(SCIENCE_TECH_EVENTS.map(e => e.id));
  const newSlugs = new Set(SCIENCE_TECH_EVENTS.map(e => e.slug));
  const filteredExisting = existingEvents.filter(e => !newIds.has(e.id) && !newSlugs.has(e.slug));

  const allEvents = [...SCIENCE_TECH_EVENTS, ...filteredExisting];
  ...
  try {
    for (const event of SCIENCE_TECH_EVENTS) {
      const dbEvent = {
        slug: event.slug,
        title: event.title,
      ...
      if (res.ok) {
        console.log(`✓ Upserted to Supabase: ${event.title}`);
      } else {
  ...
  console.log('All Science & Tech events synchronized!');
}
